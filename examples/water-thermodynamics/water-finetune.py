...
def main(args_dict: dict):
    def hparams():
        ...
        hparams.model.optimizer = MC.AdamWConfig(
            ...
            eps=1.0e-8,
            weight_decay=0.1,
            per_parameter_hparams=get_jmp_s_lr_decay(args_dict["lr"])
            if "jmp" in args_dict["model_type"]
            else None,
        )
        if args_dict["lr_scheduler"] == "steplr":
            hparams.model.lr_scheduler = MC.StepLRConfig(step_size=10, gamma=0.9)
        ...
        hparams.data.train.src = (
            "examples/water-thermodynamics/data/train_water_1000_eVAng.xyz"
        )
        ...
        hparams.data.validation.src = (
            "examples/water-thermodynamics/data/val_water_1000_eVAng.xyz"
        )
            ...
            "energy": [
                MC.PerAtomReferencingNormalizerConfig(
                    per_atom_references=Path(
                        "examples/water-thermodynamics/data/water_1000_eVAng-energy_reference.json"
                    )
                ),
        ...
        hparams.trainer.strategy = (
            DDPStrategy(find_unused_parameters=True)
            if not "orb" in args_dict["model_type"]
            else DDPStrategy(static_graph=True, find_unused_parameters=True)
        )
    ...
    ckpt_path = (
        f"./checkpoints/{args_dict['model_type']}-best-{args_dict['train_down_sample']}"
    )
    if args_dict["down_sample_refill"]:
        ckpt_path += "-refill"
    if args_dict["conservative"]:
    ...
    calc = ft_model.ase_calculator(device=f"cuda:{args_dict['devices'][0]}")
    energies_per_atom = []
    forces = []
    ...
    e_mae = torch.nn.L1Loss()(
        torch.tensor(energies_per_atom), torch.tensor(pred_energies_per_atom)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_type", type=str, default="orb-v3-conservative-inf-omat"
    )
